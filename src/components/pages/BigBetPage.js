...
class BigBetPagejs extends Component {
  ...
  killBet(x) {
    this.contracts["FOOT0Swap"].methods.killBig.cacheSend(x, {
  ...
  radioHomePick(match) {
    this.setState(state => ({
      ...state, teamTake: false, teamPick: 0,
      matchPick: match
    }));
  }
  ...
  radioAwayPick(match) {
    this.setState(state => ({
      ...state, teamTake: false, teamPick: 1,
      matchPick: match
    }));
  }
  ...
  render() {
    ...
    Object.keys(this.bigBetHistory2).forEach((index) => {
      //    if (this.checker2) {
      this.bigBetHistory2[index].forEach((bet, index) => {
        let bigBet = {
